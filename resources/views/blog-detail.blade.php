        ...
        <div class="lg:col-span-1" x-data="{
            ...
            <div class="mb-6">
                <div class="relative">
                    ...
                    <input type="text" 
                           x-model="searchQuery"
                           @input.debounce.500ms="searchBlogs()"
                    ...
                    <template x-if="blogs.length > 0">
                        <div>
                            <template x-for="blog in blogs" :key="blog.id">
                                <a :href="`/blog/${blog.id}`" class="block group pb-4 border-b border-gray-200 last:border-b-0 last:pb-0 mt-4">
                                    <p class="text-xs text-gray-500 mb-2" x-text="`${formatDate(blog.date)} • Oleh ${blog.user_created_by || 'Admin'}`"></p>
                                    <h4 class="text-sm font-semibold text-gray-900 line-clamp-2 group-hover:text-yellow-600 transition-colors" x-text="blog.title"></h4>
                                </a>
                            </template>
                        </div>
                    </template>
    ...
    <div class="mt-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($relatedBlogs as $related)
                <a href="{{ route('blog.show', $related['id']) }}" class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                    <div class="relative">
                    ...
                    <div class="p-4">
                        <span class="inline-block px-3 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-full mb-2">
